...
async function generateBriefText(input) {
    ...
    if (!key)
        throw new Error("ANTHROPIC_API_KEY (studio system key) is not configured");
    const prompt = `Buat design brief interior dari data lead berikut (Bahasa Indonesia, ringkas, actionable):\n` +
...
export const startBriefing = action({
    ...
    handler: async (ctx, { projectId }) => {
        ...
        if (client?.phone) {
            ...
            await sendWhatsAppText(client.phone, `Update untuk project "${project.title}" (${project.code}):\n` +
