...
const GATES: Record<(typeof ORDER)[number], "none" | "client_approval" | "payment" | "internal_qc"> = {
...
export const clearGate = mutation({
  ...
  handler: async (ctx, { projectId, stage }) => {
    const state = await ctx.db
      ...
      .unique();
    if (!state) throw new Error("Stage state not found");
...
export const approve = mutation({
  ...
  handler: async (ctx, { projectId, stage, kind, approvedBy, comment }) => {
    ...
    const state = await ctx.db
      ...
      .withIndex("by_project_stage", (q) => q.eq("projectId", projectId).eq("stage", stage))
      .unique();
    if (state) await ctx.db.patch(state._id, { gateClearedAt: Date.now(), status: "approved" });
  },
});
...
export const advanceStage = mutation({
  ...
  handler: async (ctx, { projectId }) => {
    ...
    const current = project.currentStage as (typeof ORDER)[number];
    ...
    if (!next) throw new Error("Project is already at the final stage");

    if (state) await ctx.db.patch(state._id, { status: "done", completedAt: Date.now() });
